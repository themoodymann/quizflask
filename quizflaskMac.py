...
#ScreenScanNumber = 3 # how often to scan the screen when looking for correct result
onMac = True
'''************************************************************ Webpage Calls *****************************
...
def getAutoAnswer(click):
    ...
    result = pyautogui.screenshot('data/firstpic.png', region=allanswers)
    ...
    if onMac:
        sleep(0.2) # <---- change this until it works...? may depend on whether answer was correct or wrong, and possibly even location of answer...?
    else:
        sleep(0.3)
    #result = pyautogui.screenshot(region=allanswers)
    result = pyautogui.screenshot('data/secondpic.png',region=allanswers)
    newgreens = testColor(result)
    print("screen scanning time = ",time.time()-beforeclick)
    shifts = [a_i - b_i for a_i, b_i in zip(newgreens, originalgreens)]
